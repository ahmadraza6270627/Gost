...
// Voice notes are also saved to backend DB.
...
async function loadHistory(topic) {
  try {
    ...
    history.forEach(m => {
      ...
      if (m.type === 'voice') {
        renderVoiceNote(m.audio, m.mimeType, isMe, m.durationMs);
        return;
      }
...
function saveVoiceNote(topic, audio, mimeType, durationMs) {
  fetch(`${MESSAGES_API}/messages`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json'
...
function sendVoiceNote(base64, mimeType, durationMs) {
  ...
  try {
    ...
    saveVoiceNote(currentTopic, base64, mimeType, durationMs);
  } catch (err) {
